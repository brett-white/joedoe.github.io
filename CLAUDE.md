...
Then open `http://localhost:8080/index.html` or `http://localhost:8080/partners.html`.
...
- `partners.html` — a separate page for the partners/investors audience (BaaS & rails partners, HRIS integrators, strategic investors; market sizing; roadmap).
- `styles.css` — single shared stylesheet for both pages.
- `assets/` — brand SVGs (icon, stacked logo, stacked wordmark, wide wordmark).

There is no templating layer: the header nav and footer markup are duplicated by hand in both HTML files, including an inline copy of the wordmark SVG in each (rather than referencing the file in `assets/`). When editing the logo, nav links, or footer, update both `index.html` and `partners.html` to keep them in sync.
...
Both pages share one nav structure but link to each other for cross-page sections, e.g. `partners.html` links back to `index.html#pricing`. When renaming or removing a section `id` in one file, check the other file for cross-page anchor links pointing at it (`grep -rn 'href="index.html#' partners.html` / `grep -rn 'href="partners.html#' index.html`).
